...
function sliderInit(el, textfade) {
  ...
  function pad(str, max) {
	...
	function linkChange() {
		...
		slider.siblings().find('a.lead__footerlink').attr('href', detailsLink);
	}
	linkChange();
...
$('.video__promo').click(function(ev){
	$(this).addClass('active');
	$(this).find('.video__frame')[0].src += "&autoplay=1";
    ev.preventDefault();
});
...
sr.reveal('h1, .service__content, .review__content p, .page-intro p, .text-list__list li, .mission, .page-menu, .news__tabs-item, .news__title time, .post__header time, .support__phone, .support__email, .support__answer', fooReveal1);
